...
class TransformerLayer(Module):
    ...
    def forward(self, x):
        ...
        ### BEGIN YOUR SOLUTION
        _x = x
        x = self.ln_1(x.view(batch_size*seq_len, n_embd)).view(batch_size, seq_len, n_embd)
        x = self.attention(x)
        x += _x

        _x = x
        x = self.ln_2(x.view(batch_size*seq_len, n_embd)).view(batch_size, seq_len, n_embd)
        x = self.ff(x)
        x += _x
